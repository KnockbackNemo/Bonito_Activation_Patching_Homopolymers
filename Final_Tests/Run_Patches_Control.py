...
def check_if_run_exists(file_name, read_idx, row_idx, component, timestamps):
    ...
    for step in timestamps: ## Todo currently doesn't include -1
...
for current_read_idx, read_data in enumerate(reads, start=1):
    ...
    for index, row in df_inputpairs.iterrows():
        ...
        corruption_recorded_start_raw = safe_parse(row['Corruption Start Raw'], int)
        corruption_recorded_end_raw = safe_parse(row['Corruption End Raw'], int)
