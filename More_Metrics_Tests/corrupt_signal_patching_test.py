...
# To run this folder, you'll need saved tensors for the clean and corrupt inputs
import torch
import os
import gc
import pod5
...
FILE_NAME_EXTENSION = "Synth_Splice_1"
...
# Load in data
input = torch.load('clean_tensor.pt')
corrupt_input = torch.load('corrupt_tensor.pt')
...
# Chop data
start_idx = 0 #54250 # Skip beginning noise
LENGTH = len(input[0, 0, :])#250
end_idx = start_idx + LENGTH
raw_stndrd_signal = input#first_read.signal
chopped_signal = raw_stndrd_signal[0, 0, start_idx:end_idx]

clean_signal = chopped_signal.detach().clone()
...
# Change this to pure difference but still look at global mse separately
##############################
###### PATCHING SWEEP ########
##############################
