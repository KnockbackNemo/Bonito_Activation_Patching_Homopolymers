...
for num_read in range(1, highest_read + 1): # highest_read = 2 means run this twice
    ...
    while chop_request.loc["read_num"] == num_read: # Chop out all the sections we want from this read
        ...
        for extend in range(0, 2): 
            ...
            plateau_start = INPUT_BEGIN_PADDING_LENGTH # This is when the first token should be since everything is relative to it
            plateau_end = next_tok_start
            plateau_signal = input[0, 0, plateau_start:plateau_end]

            tick_delta = 6 # input/output length ratio

            # Prevent negatives in case it is shorter than tick_delta anincrease frequency of signal
            new_length = max(1, len(plateau_signal) - tick_delta) 
            squished = F.interpolate(plateau_signal.view(1, 1, -1), size=new_length, mode='linear', align_corners=False).squeeze()
            
            if extend: # Move the signal forward and add a "hold" signal
                       
                corrupt_input[0, 0, plateau_start : plateau_start + new_length] = squished
                
                # Fill in gap with a flatline to simulate "blank" hold
                corrupt_input[0, 0, plateau_start + new_length : plateau_end] = plateau_signal[-1]

            else: # Shorten by filling with noise instead
              
                corrupt_input[0, 0, plateau_start : plateau_start + new_length] = squished
                
                # Fill the gap with ambient noise from right before the plateau
                noise = input[0, 0, max(0, plateau_start - tick_delta) : plateau_start]
                
                # Check that the noise size and gap size match
                gap_size = plateau_end - (plateau_start + new_length)
                if len(noise) < gap_size:
                    # If we hit the start of the array, just pad with zeros
                    noise = F.pad(noise, (0, gap_size - len(noise)))
                    
                corrupt_input[0, 0, plateau_start + new_length : plateau_end] = noise[:gap_size]
