# Noising! This is pretty much a copy of the other.
...
heatmap_data_22 = np.zeros((NUM_T_LAYERS, NUM_TIMESTEP_SWEEPS))
heatmap_data_23 = np.zeros((NUM_T_LAYERS, NUM_TIMESTEP_SWEEPS))
heatmap_data_24 = np.zeros((NUM_T_LAYERS, NUM_TIMESTEP_SWEEPS))
...
for layer_idx in range(NUM_T_LAYERS):
    ...
    for time_offset, t in enumerate(range(SWEEP_WINDOW_START, SWEEP_WINDOW_END)):
        ...
        ################ Some things because I'm too tired to think right now.
        Target_transition = torch.argmax(clean_output[])
